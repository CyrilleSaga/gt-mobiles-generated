...
class TransactionResourceApi {
  ...
  TransactionResourceApi([ApiClient? apiClient]) : apiClient = apiClient ?? defaultApiClient;

  /// createTransaction
  ///
  ///
  Future<TransactionDTO?> createTransactionUsingPOST({required TransactionDTO body}) async {
    Object postBody = body;

    // verify required params are set

    // create path and map variables
    String path = "/api/transactions".replaceAll("{format}", "json");
    ...
    if (response.statusCode >= 400) {
      ...
      return apiClient.deserialize(response.body, 'TransactionDTO') as TransactionDTO;
    } else {
      return null;
    }
  }
  ...
  ///
  ///
  Future deleteTransactionUsingDELETE(int id) async {
    // verify required params are set
    if (id == null) {
    ...
    // create path and map variables
    String path = "/api/transactions/{id}".replaceAll("{format}", "json").replaceAll("{" + "id" + "}", id.toString());
  ...
  Future<List<TransactionDTO>?> getAllTransactionsUsingGET({String? filter, int? page, int? size, List<String>? sort}) async {
    ...
    if (response.statusCode >= 400) {
      ...
      return (apiClient.deserialize(response.body, 'List<TransactionDTO>') as List).map((item) => item as TransactionDTO).toList();
    } else {
      return null;
    }
  }
  ...
  ///
  ///
  Future<TransactionDTO?> partialUpdateTransactionUsingPATCH(int id, {required TransactionDTO body}) async {
    Object postBody = body;
  ...
  ///
  ///
  Future<TransactionDTO?> updateTransactionUsingPUT(int id, {required TransactionDTO body}) async {
    Object postBody = body;
